...
export async function runReconcileAfterRebuild(params: {
  ...
  try {
    const r2: any = await apiTry("Reconcile 提醒(memo)", () => (api as any).rslatteItemsReconcile("memo", { scope_file_paths, present_uids: memoUids, present_ids: [] }));
...
export async function runReconcileForType(params: {
  ...
  try {
    const r: any = await apiTry(
      itemType === "task" ? "Reconcile 任务(task)" : "Reconcile 提醒(memo)",
...
/**
 * Reconcile schedule rows under scope_file_paths against schedule-index scan (present uids).
 * Same safety gates as runReconcileForType. Empty present_uids is allowed (backend treats as “none present”).
 */
export async function runReconcileSchedule(params: {
  enableSync: boolean;
  api: RSLatteApiClient;
  queue: SyncQueue;
  requireQueueEmpty: boolean;
  ...
  const { enableSync, api, queue, requireQueueEmpty, requireFileClean, includedFilePaths, lines, dbg } = params;
  if (!enableSync) return;
  ...
  try {
    const r: any = await apiTry("Reconcile 日程(schedule)", () =>
      (api as any).schedulesReconcile({ scope_file_paths, present_uids })
    );
    dbg?.("taskRSLatte", "reconcile schedule done", r);
    new Notice(`Reconcile(schedule) 完成：标记删除 ${(r?.updated ?? 0)} 条`);
  } catch (e: any) {
    new Notice(`Reconcile(schedule) 失败：${e?.message ?? String(e)}`);
  }
}
