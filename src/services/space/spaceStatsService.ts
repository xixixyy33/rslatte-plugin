...
export class SpaceStatsService {
  ...
  private isModuleEnabled(moduleKey: RSLatteModuleKey, ctx: SpaceCtx): boolean {
    ...
    // 与 Pipeline 同源；缺省保守为关（§0.4）
    return (this.plugin as any).isPipelineModuleEnabled?.(moduleKey) ?? false;
  }
  ...
  private isModuleDbSyncEnabled(moduleKey: RSLatteModuleKey, ctx: SpaceCtx): boolean {
    ...
    if (ctx.space?.settingsSnapshot) {
      ...
      switch (moduleKey) {
        ...
        case "health":
          return !!s?.healthPanel?.enableDbSync;
        case "knowledge":
          return !!s?.knowledgePanel?.enableDbSync;
        default:
          return false;
      }
    }
  ...
  private async readQueueCountsForModule(
    ...
    try {
      ...
      if (moduleKey === "task" || moduleKey === "memo" || moduleKey === "schedule") {
      ...
      // checkin/finance/health: 从 recordRSLatte 索引快照读取（与 recordSync 哈希策略对齐）
      if (moduleKey === "checkin" || moduleKey === "finance" || moduleKey === "health") {
        ...
        if (recordRSLatte) {
          try {
            ...
            if (moduleKey === "checkin") {
              ...
              const snap = await recordRSLatte.getHealthSnapshot(false);
              const items = (snap.items ?? []) as any[];
              let pending = 0;
              let failed = 0;
              ...
              for (const it of items ?? []) {
                ...
                if (needsSync(it, computeHealthHash)) {
                  pending++;
                }
              }
      ...
      if (moduleKey === "knowledge") {
        const meta = (this.plugin as any)._dbSyncMeta?.knowledge;
        return {
          pending: Number(meta?.pendingCount ?? 0),
          failed: Number(meta?.failedCount ?? 0),
        };
      }
  ...
  ): Promise<{ counts: any; kpi: RSLatteModuleKpiByModule }>
  {
    ...
    if (moduleKey === "memo") {
      ...
      let overdueUnclosed = 0;
      let dueWithin7dUnclosed = 0;
      for (const it of active as RSLatteIndexItem[]) {
        const st = String((it as any).status ?? "").toUpperCase();
        if (st === "DONE" || st === "CANCELLED") continue;
        const d = String((it as any).memoDate ?? "").slice(0, 10);
        if (!d || d.length !== 10) continue;
        if (d < t) overdueUnclosed++;
        else if (ymdBetween(d, t, next7End)) dueWithin7dUnclosed++;
      }

      return {
        counts,
        kpi: { memo: { total: active.length, new7d, overdueUnclosed, dueWithin7dUnclosed } },
      };
    }
    ...
    const overdue = activeTasks.filter((it) => {
      ...
      const due = String((it as any).planned_end ?? "");
  ...
  /** 日程：从 schedule-index 聚合 H（未来 7 天块时长之和，小时）、E（已过结束时刻仍未闭环块数） */
  private async computeScheduleCountsAndKpi(ctx: SpaceCtx): Promise<{ counts: any; kpi: RSLatteModuleKpiByModule }>
  {
    const app = (this.plugin as any).app;
    const s: any = (this.plugin as any).settings;
    const spaceIndexDir = resolveSpaceIndexDir(s, ctx.spaceId, [s?.taskPanel?.rslatteIndexDir]);
    const fallback: RSLatteIndexFile = { version: 1, updatedAt: new Date().toISOString(), items: [] };
    const idx = await this.readSpaceIndexFile(app, spaceIndexDir, "schedule-index.json", fallback);
    const items = Array.isArray((idx as any)?.items) ? ((idx as any).items as any[]) : [];
    const archived = items.filter((it) => (it as any)?.archived === true);
    const active = items.filter((it) => (it as any)?.archived !== true);
    const counts = { total: items.length, active: active.length, archived: archived.length };
    ...
    const t = todayYmd();
    const tEnd = ymdAdd(t, 7);
    const nowMs = Date.now();
    let scheduledHoursNext7d = 0;
    let expectedUnclosedCount = 0;

    for (const it of active) {
      const st = String((it as any).status ?? "").toUpperCase();
      if (st === "DONE" || st === "CANCELLED") continue;
      const extra = ((it as any).extra ?? {}) as Record<string, unknown>;
      const sd = String(extra.schedule_date ?? (it as any).memoDate ?? "").slice(0, 10);
      const startTime = String(extra.start_time ?? "").trim();
      const durationMin = Math.max(1, num(extra.duration_min) || 60);

      if (!/^\d{4}-\d{2}-\d{2}$/.test(sd)) continue;

      if (ymdBetween(sd, t, tEnd)) {
        scheduledHoursNext7d += durationMin / 60;
      }

      const startM = momentFn(`${sd} ${startTime || "00:00"}`, "YYYY-MM-DD HH:mm", true);
      const startMs = startM.isValid() ? startM.valueOf() : momentFn(sd, "YYYY-MM-DD", true).valueOf();
      if (!Number.isFinite(startMs) || startMs <= 0) continue;
      const endMs = startMs + durationMin * 60 * 1000;
      if (endMs < nowMs) expectedUnclosedCount++;
    }

    return {
      counts,
      kpi: { schedule: { scheduledHoursNext7d, expectedUnclosedCount } },
    };
  }

  /** 健康：仅汇总索引条数；定级与告警仍主要依赖分析索引（Hub 告警区已单独拉取） */
  private async computeHealthCountsAndKpi(ctx: SpaceCtx): Promise<{ counts: any; kpi: RSLatteModuleKpiByModule }>
  {
    const app = (this.plugin as any).app;
    const s: any = (this.plugin as any).settings;
    const spaceIndexDir = resolveSpaceIndexDir(s, ctx.spaceId, [s?.rslattePanelIndexDir]);
    const snap = await this.readSpaceIndexFile<any>(
      app,
      spaceIndexDir,
      "health-record-index.json",
      { version: 1, updatedAt: new Date().toISOString(), items: [] }
    );
    const items = Array.isArray(snap?.items) ? snap.items : [];
    const valid = items.filter((r: any) => !r?.isDelete);
    return { counts: { total: valid.length, active: valid.length, archived: 0 }, kpi: {} };
  }
  ...
  private async computeProjectCountsAndKpi(ctx: SpaceCtx): Promise<{ counts: any; kpi: RSLatteModuleKpiByModule }>
  {
    ...
    const dueNext14d = active.filter((it) => {
      const due = String((it as any).due_date ?? (it as any).planned_end ?? "");
  ...
  private async computeCountsAndKpi(ctx: SpaceCtx, moduleKey: RSLatteModuleKey): Promise<{ counts: any; kpi: RSLatteModuleKpiByModule }>
  {
    ...
    if (moduleKey === "schedule") return this.computeScheduleCountsAndKpi(ctx);
    if (moduleKey === "health") return this.computeHealthCountsAndKpi(ctx);
  ...
  public async refreshSpaceStats(ctx: SpaceCtx, enabledModules?: RSLatteModuleKey[], force?: boolean): Promise<void>
  {
    ...
    const modules: RSLatteModuleKey[] = enabledModules && enabledModules.length
      ? enabledModules
      : (["task", "memo", "schedule", "checkin", "finance", "health", "project", "output", "contacts"] as any);
