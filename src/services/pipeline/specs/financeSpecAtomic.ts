...
function computeFinanceGateFromMeta(plugin: any, dbSyncEnabled: boolean): RSLatteReconcileGate {
  const gate: RSLatteReconcileGate = { dbSyncEnabled };
  if (!dbSyncEnabled) return gate;
  try {
    const meta: any = plugin?._dbSyncMeta?.finance ?? {};
...
export function createFinanceSpecAtomic(plugin: any): ModuleSpecAtomic {
  // runId-scoped flag so we only clear dbSyncForceFullNext.finance after a successful flush
  ...
  const computeDbSyncEnabled = (): boolean => {
    try {
      return plugin?.isFinanceDbSyncEnabled?.() === true;
    } catch {
      return false;
    }
  };
  ...
  const getForceFullFlag = (): boolean => {
    try {
      return !!(plugin?.settings as any)?.dbSyncForceFullNext?.finance;
    } catch {
      return false;
    }
  };
  ...
  const ensureBackendSafe = async (): Promise<boolean> => {
    try {
      ...
      const ok2 = await plugin?.vaultSvc?.ensureVaultReadySafe?.("financeSpecAtomic");
      return ok2 !== false;
    } catch {
      return false;
    }
  ...
  return {
    ...
    async scanIncremental(_ctx) {
      try {
        await plugin?.recordRSLatte?.ensureReady?.();
        const scan: any = await plugin?.recordRSLatte?.scanIncrementalFromDiary?.({ modules: FINANCE_MODULES });
        return ok(scan ?? { empty: true, modules: FINANCE_MODULES, kind: "incremental" });
      } catch (e: any) {
        return fail("FIN_SCAN_INCREMENTAL_FAILED", "Finance scanIncremental failed", { message: e?.message ?? String(e) });
      }
    },
    ...
    async archiveOutOfRange(ctx) {
      ...
      try {
        await plugin?.recordRSLatte?.ensureReady?.();
        const r: any = await plugin?.recordRSLatte?.archiveNow?.(FINANCE_MODULES);
        const archivedCount = Number(r?.financeArchived ?? 0);
        const listsArchived = Number(r?.listsArchivedFinance ?? 0);
        return ok(
          mkSummary(
        ...
        return fail("FIN_ARCHIVE_FAILED", "Finance archiveOutOfRange failed", { message: e?.message ?? String(e) });
      }
    },
    ...
    async buildOps(ctx, applied: any) {
      ...
      try {
        ...
        await plugin?.syncRecordListsToDbNow?.(forceFullSync || ctx.mode === "rebuild", { modules: FINANCE_MODULES });
        return ok({ listsSynced: 1, forceFullSync } as any);
      } catch (e: any) {
        // 不阻断：失败只影响 DB sync，不影响本地
        console.warn(`[RSLatte][finance][${ctx.mode}] syncRecordListsToDbNow failed`, e);
    ...
    async flushQueue(ctx, _opts: RSLatteFlushQueueOptions) {
      ...
      try {
        ...
        const reason = forceFullSync ? "manual_rebuild_finance" : String(ctx.mode);
        await plugin?.syncRecordIndexToDbNow?.({ reason, modules: FINANCE_MODULES });
        ...
        if (ctx?.runId && forceFullByRunId.get(ctx.runId) === true) {
          ...
          await plugin?.consumeForceFullFlag?.("finance", true);
        }
        ...
        console.warn(`[RSLatte][finance][${ctx.mode}] syncRecordIndexToDbNow failed`, e);
    ...
    async getReconcileGate(_ctx) {
      ...
      const gate = computeFinanceGateFromMeta(plugin, dbSyncEnabled) as any;
    ...
    async reconcile(ctx) {
      const startedAt = new Date().toISOString();
      const dbSyncEnabled = computeDbSyncEnabled();
      const gate = computeFinanceGateFromMeta(plugin, dbSyncEnabled);
      
      // ✅ DEBUG: 记录 reconcile 开始
      const debugLogEnabled = (plugin?.settings as any)?.debugLogEnabled === true;
      if (debugLogEnabled) {
        console.log(`[RSLatte][finance][reconcile] Starting reconcile: mode=${ctx.mode}, dbSyncEnabled=${dbSyncEnabled}`);
      }
      
      try {
        if (!dbSyncEnabled) {
          if (debugLogEnabled) {
            console.log(`[RSLatte][finance][reconcile] Skipping: dbSyncEnabled=false`);
          }
        ...
        if (ctx.mode === "manual_refresh") {
          try {
            ...
            if (debugLogEnabled) {
              ...
              console.log(`[RSLatte][finance][reconcile] manual_refresh: Starting incremental scan, lastScanMs=${new Date(lastScanMs).toISOString()} (${lastScanMs})`);
            }
            
            // ✅ manual_refresh 模式下，使用 forceIncludeEqualMtime=true 来扫描 mtime === sinceMs 的文件
            // ✅ 这样可以处理 Obsidian 缓存或文件系统时间精度问题，确保手动刷新时能检测到所有可能的变化
            incrementalScan = await plugin?.recordRSLatte?.scanIncrementalFromDiary?.({ modules: FINANCE_MODULES, forceIncludeEqualMtime: true });
            
            // ✅ DEBUG: 记录增量扫描的结果
            if (debugLogEnabled) {
              console.log(`[RSLatte][finance][reconcile] manual_refresh: Incremental scan result:`, {
            ...
            // ✅ 如果返回了结果（即使 dayKeysScanned 可能为空，但 scannedFiles 不为空），说明文件被修改了，需要 reconcile
            if (!incrementalScan) {
              shouldReconcile = false;
              console.log(`[RSLatte][finance][reconcile] manual_refresh: No file changes detected, skipping reconcile API call`);
              ...
              if (!hasScannedFiles && !hasParsedRecords) {
                ...
                console.warn(`[RSLatte][finance][reconcile] manual_refresh: Incremental scan returned empty result, skipping reconcile`);
                shouldReconcile = false;
                r = { scannedDays: 0, clearedDays: 0, cutoffDate: null, dayKeysScanned: new Set<string>(), dayKeysToReplace: new Set<string>() };
              } else {
                ...
                if (hasParsedRecords) {
                  console.log(`[RSLatte][finance][reconcile] manual_refresh: Records detected (scannedDays=${incrementalScan.dayKeysScanned?.size ?? 0}), applying incremental scan first`);
                  ...
                  console.log(`[RSLatte][finance][reconcile] manual_refresh: Files modified but no records found (possible deletions, scannedDays=${incrementalScan.dayKeysScanned?.size ?? 0}), updating lastDiaryScanMs and using incremental scan result for reconcile`);
                  ...
                  // ✅ 对于删除的情况，仍然需要对增量扫描检测到的日期进行全量重建，以确保索引状态正确
                  // ✅ 但只对这些日期进行 reconcile
        ...
        if (!api?.financeRecordsReconcile) {
          console.warn(`[RSLatte][finance][reconcile] financeRecordsReconcile API not available`);
        } else {
          try {
            // 获取当前索引中的财务记录，构建 present_comp_keys
            const fsnap = await plugin?.recordRSLatte?.getFinanceSnapshot?.(false);
            ...
            if (debugLogEnabled) {
              console.log(`[RSLatte][finance][reconcile] manual_refresh: Using scanned dates for reconcile:`, {
                mode: ctx.mode,
            ...
            console.log(`[RSLatte][finance][reconcile] Debug: scannedDates.length=${scannedDates.length}, presentCompKeys.size=${filteredPresentCompKeys.size} (filtered from ${presentCompKeys.size}), dayKeysScanned.size=${dayKeysScanned?.size ?? 0}, dayKeysToReplace.size=${dayKeysToReplace?.size ?? 0}`);
            ...
            if (scannedDates.length > 0 || filteredPresentCompKeys.size > 0) {
              ...
              console.log(`[RSLatte][finance][reconcile] Calling financeRecordsReconcile API with payload:`, {
              ...
              const reconcileResp: any = await api.financeRecordsReconcile(reconcilePayload);
              const markedDeleted = Number(reconcileResp?.marked_deleted ?? 0);
              const keep = Number(reconcileResp?.keep ?? 0);
              
              console.log(`[RSLatte][finance][reconcile] API response: marked_deleted=${markedDeleted}, keep=${keep}`);
              ...
              // ✅ 调试日志：记录为什么没有调用 API
              console.warn(`[RSLatte][finance][reconcile] Skipping API call: scannedDates.length=${scannedDates.length}, presentCompKeys.size=${presentCompKeys.size}`);
            }
          } catch (e: any) {
            // ✅ 如果后端 reconcile 失败，记录警告但不阻断流程
            console.warn(`[RSLatte][finance][reconcile] financeRecordsReconcile failed:`, e);
          }
        }
        ...
        return fail("FIN_RECONCILE_FAILED", "Finance reconcile failed", { message: e?.message ?? String(e) });
      }
    },
